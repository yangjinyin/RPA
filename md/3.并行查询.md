...
1. `JOIN_TAB *join_tab{nullptr}`：指向一个 `JOIN_TAB` 结构的数组，每个元素对应查询中一个参与连接的表。优化器的主要工作

​       数组长度为 `tables`（查询涉及的表数量）。

​       每个 `JOIN_TAB` 包含：  

- 表信息（`TABLE *table`）
- 访问方法（`type`：如 `JT_ALL`, `JT_REF`, `JT_RANGE`）
- 条件（`cond`, `select_cond`）
- 索引选择、代价估算、物化信息等



2. `QEP_TAB *qep_tab{nullptr}`：指向一个 `QEP_TAB` 结构的数组，每个元素对应执行阶段的一个表。

| JOIN_TAB                | QEP_TAB                            |
| ----------------------- | ---------------------------------- |
| 优化阶段使用            | 执行阶段使用                       |
| 包含 cost、候选访问路径 | 包含最终选定的执行函数             |
| 可能包含多个 plan 选项  | 只保留最优 plan 的执行逻辑         |
| 用于比较不同 join order | 用于实际调用 read_first, read_next |



3. `JOIN_TAB **best_ref{nullptr}`：指向一个指针数组，数组中的每个元素是 `JOIN_TAB`；这个数组表示当前找到的 **最优连接顺序（best join order）**

4. `JOIN_TAB **map2table{nullptr}`：表索引 → JOIN_TAB” 的映射表；`map2table[i]` 返回第 `i` 个表对应的 `JOIN_TAB`。 实际上，`map2table[i]` 通常等于 `&join_tab[i]`，但设计为指针数组是为了灵活性（例如物化视图或派生表可能不同）
