...
```c++
/*pipeline: 表示数据处理的管道，包含了操作符和接收端信息。
  source_chunk: 当前处理的数据块，可能是从中间结果或最终结果获取的。
  chunk_budget: 用于管理当前的执行预算，限制每个执行循环中处理的数据量。
  remaining_sink_chunk: 标记是否还有待处理的接收块。
  exhausted_source: 标记数据源是否已经耗尽。
  done_flushing: 标记是否已完成所有操作的刷新过程。
  next_batch_blocked: 标记获取下一个批次数据是否被阻塞。
  in_process_operators: 当前正在处理的操作符列表。*/
PipelineExecuteResult PipelineExecutor::Execute(idx_t max_chunks) {
	...
	//一切完成，则调用 PushFinalize() 方法进行最终处理	
	return PushFinalize();
}
```
...
```c++
/* input: 输入的数据块，即需要经过管道处理的原始数据
 * chunk_budget: 用于管理执行过程中的预算，以限制在某一时间内可以处理的数据量
 * final_chunk: 存放最终处理结果的数据块
 * sink_chunk: 指向 final_chunk，用于向接收端推送结果的信息
 */
OperatorResultType PipelineExecutor::ExecutePushInternal(DataChunk &input, ExecutionBudget &chunk_budget,
	...
	//循环执行，直到 OperatorResultType 为 HAVE_MORE_OUTPUT； 或者接收端被阻塞；或者执行预算耗尽
	OperatorResultType result = OperatorResultType::HAVE_MORE_OUTPUT;
	do {
		//如果当前处理的输入不是 final_chunk，则重置 final_chunk 并执行管道操作，将结果放入 final_chunk
        ...
        //检查 final_chunk 的大小，如果大于零，准备将其推送到接收端（Sink）
		if (sink_chunk.size() > 0) {
			StartOperator(*pipeline.sink);
			D_ASSERT(pipeline.sink);
			D_ASSERT(pipeline.sink->sink_state);
			OperatorSinkInput sink_input {*pipeline.sink->sink_state, *local_sink_state, interrupt_state};
			//调用 Sink 函数将 sink_chunk 传递到接收端，根据返回的结果进行处理：
			//	 	如果返回 BLOCKED，表示接收端无法接收更多数据，返回相应的状态。
			//		如果返回 FINISHED，表示处理完成，调用 FinishProcessing 进行清理并返回完成状态。
...
![duckdb-executor_2.png](https://leviathan.vip/images/duckdb-executor_2.png)
