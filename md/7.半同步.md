...
flush阶段 binlog flush完成后，log cache会更新pos，此时被dump线程获取到get_binlog_end_pos()

```cpp
|--Binlog_sender::run()
|    |--Binlog_sender::send_binlog(&log_cache, start_pos)
|    |        |--get_binlog_end_pos(log_cache) // 获取binlog最后的位置，如果没有更新会一直卡在这面
|    |        |--Binlog_sender::send_events(IO_CACHE *log_cache, my_off_t end_pos) // 循环发送events
|    |        |      |--Binlog_sender::read_event() // 以event为单位读取
|    |        |      |--before_send_hook(log_file, log_pos)// 回掉函数 半同步 + header 后发送
|    |        |      |    |--repl_semi_before_send_event() 
|    |        |      |--send_packet()  // 写入到net_buff里面
|    |        |      |--after_send_hook() // 回调函数
|    |        |      |    |--repl_semi_after_send_event()
|    |        |      |    |    |--ReplSemiSyncMaster::readSlaveReply() //log中日志：[Note] ---> ReplSemiSyncMaster::readSlaveReply
|    |        |      |    |    |--net_flush() //net_buff发送
```

﻿

备机处理逻辑 handle_slave_io：

```cpp
|--handle_slave_io()
|   |--loop //while (!io_slave_killed(thd,mi)) 
|   |--read_event()
|   |   |--my_net_read() //不停的去读网络
|   |--RUN_HOOK(binlog_relay_io, after_read_event)//读到event了，调用回调函数repl_semi_slave_read_event 来解析header
|   |   |--ReplSemiSyncSlave::slaveReadSyncHeader() 
|   |--queue_event() //处理event，放到队列里
|   |--RUN_HOOK(binlog_relay_io, after_queue_event)// 回掉函数 ReplSemiSyncSlave::slaveReply() 如果是最后一个event，需要回ack 则回复
...
int MYSQL_BIN_LOG::ordered_commit(THD *thd, bool all, bool skip_commit)
{
  ...
  // 将flush阶段的链表里面所有的事物日志写入到binlog的buff里面，这面也会生成gtid_event，所以解析binlog会发现gtid_event时间是在后面query_event后面
  flush_error= process_flush_stage_queue(&total_bytes, &do_rotate, &wait_queue);
  // 将binlog刷新到文件缓存里面
  flush_error= flush_cache_to_file(&flush_end_pos);
  // 判断sync_binlog是否为1
  update_binlog_end_pos_after_sync= (get_sync_period() == 1); 
  // 观察者模式，调用Binlog_storage_observer里面的repl_semi_report_binlog_update函数，将文件名和pos点记录下来
  ...
  // update_binlog_end_pos_after_sync不为1，这面就更新binlog文件的最后位置点，唤醒dump线程发送binlog
  if (!update_binlog_end_pos_after_sync)
   update_binlog_end_pos();
  
  # sync阶段 将binlog内容刷新到磁盘
  ...
  if (update_binlog_end_pos_after_sync)
  {
    ...
    // 更新binlog文件的最后位置点，唤醒dump线程发送binlog
    update_binlog_end_pos(tmp_thd->get_trans_pos());
  }
  
  # commit阶段 提交所有的事务
  // 如果rpl_semi_sync_master_wait_point的值设置为after_sync，在这面等待从库的ACK
  sync_error= call_after_sync_hook(commit_queue);
  // 遍历commit链表里面的事务进行提交
  process_commit_stage_queue(thd, commit_queue);
  // 如果rpl_semi_sync_master_wait_point的值设置为after_commit，在这面等待从库的ACK
